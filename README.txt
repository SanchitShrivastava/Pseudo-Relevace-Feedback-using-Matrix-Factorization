*In order to run the query, open main.py using command "python main.py" on terminal and enter the query. Click on search to get the ranked results. The dataset "d,q and r " folders should be present in the same folder as the main.py*
...
"d", "q" and "r" folders contains documents, queries and relevent results respectively. This is the dataset used by us for the experiment.
...
Intermediate results folder has files corresponding to different heuristics and methids i.e.-
	...
	noprf - A simple ranked retrieval along with average precision, p@10, p@20 for each query and then overall MAP, p@10, p@20 			calculated by taking average of the data in the csv file.
